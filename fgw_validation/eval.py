...
def _bootstrap_struct_corr_ci(C_i: np.ndarray, C_a_pi: np.ndarray,
                              ...
                              want_spearman: bool = True
                              ) -> tuple[tuple[float, float], tuple[float, float]]:
    """**Item-level** bootstrap CI of (Pearson, Spearman) for the structural
    correlation between `C_i` and `π`-aligned `C_a`.

    Resamples image indices with replacement, recomputes the upper-triangle
    correlation on the resampled n×n submatrix. Set `want_spearman=False` to
    skip the Spearman path (≈2× faster — argsort dominates the cost).

    Returns ((p_lo, p_hi), (s_lo, s_hi)). When skipped, Spearman is NaN.
    """
    n = C_i.shape[0]
    ...
    s_vals = np.empty(B, dtype=np.float64) if want_spearman else None
    pos = 0
    while pos < B:
        cur = min(chunk, B - pos)
        ...
        pos += cur
    alpha = (1.0 - ci) / 2.0
    p_ci = (float(np.quantile(p_vals, alpha)), float(np.quantile(p_vals, 1.0 - alpha)))
    if want_spearman:
        s_ci = (float(np.quantile(s_vals, alpha)), float(np.quantile(s_vals, 1.0 - alpha)))
    else:
        s_ci = nan_pair
    return p_ci, s_ci
...
def _structural_null(C_i: np.ndarray, C_a: np.ndarray,
                     K: int, ci: float, rng, lite: bool = False) -> dict:
    """Random-permutation null for `pearson_dist_corr` / `spearman_dist_corr`.

    Independent of `π`: depends only on (C_i, C_a). When `lite=True`, only
    Pearson is computed (Spearman ranks are the dominant cost in this loop).
    """
    n = C_i.shape[0]
    if n < 3 or K <= 0:
        return {}
    ...
    s_vals = None if lite else np.empty(K, dtype=np.float64)
    for k in range(K):
...
def _lexical_sim(emb_root, splits, idx_i, idx_a):
    ...
    Vectorised: build 0/1 bag-of-words matrices over a shared vocabulary,
    then compute |A∩B| via matrix product and |A∪B| via set-size identity.
    For n=200 this runs in ~50 ms vs ~1 s for the naive double loop.
    """
    ...
    if V == 0:
        return np.zeros((n_i, n_a), dtype=np.float64)

    Xi = np.zeros((n_i, V), dtype=np.float32)
    Xa = np.zeros((n_a, V), dtype=np.float32)
    for i, tset in enumerate(toks_i):
        for t in tset: Xi[i, vocab[t]] = 1.0
    for j, tset in enumerate(toks_a):
        for t in tset: Xa[j, vocab[t]] = 1.0

    inter  = Xi @ Xa.T                                   # (n_i, n_a)
    a_size = Xi.sum(axis=1, keepdims=True)               # (n_i, 1)
    b_size = Xa.sum(axis=1, keepdims=True)               # (n_a, 1)
    union  = a_size + b_size.T - inter
    return np.where(union > 0, inter / np.maximum(union, 1.0), 0.0).astype(np.float64)
...
def _structural(C_i: np.ndarray, C_a: np.ndarray, pi: np.ndarray,
    ...
    a = C_i[iu0, iu1]
    b = C_a_pi[iu0, iu1]
    out = {f"{prefix}pearson_dist_corr": _pearson(a, b)}
    if not lite:
        out[f"{prefix}spearman_dist_corr"] = _spearman(a, b)
    if B > 0:
        (p_lo, p_hi), (s_lo, s_hi) = _bootstrap_struct_corr_ci(
            C_i, C_a_pi, B, ci, rng, want_spearman=not lite,
        )
...
def _triplet_agreement(C_i, C_a, pi, n_triplets, B, ci, rng,
                       prefix: str = "", lite: bool = False) -> dict:
    """Fraction of NN-triplets preserved under π. `lite=True` skips the CI."""
    key = f"{prefix}triplet_agreement"
    out: dict = {key: float("nan")}
    want_ci = (B > 0) and (not lite)
    if want_ci:
...
def _witness_sim(emb_root, splits, idx_i, idx_a, witness: str):
    ...
    unavailable. Cached by (witness, idx_i_bytes, idx_a_bytes) — within one
    eval bundle every combo shares the same idx_i, idx_a, so each witness is
    materialised at most once per file.
    """
    idx_i_arr = np.asarray(idx_i)
    idx_a_arr = np.asarray(idx_a)
    key = (str(emb_root), splits["flickr8k"], splits["clotho"], witness,
           idx_i_arr.tobytes(), idx_a_arr.tobytes())
    if key in _WITNESS_SIM_CACHE:
        return _WITNESS_SIM_CACHE[key]
    if witness == "lex":
        sim = _lexical_sim(emb_root, splits, idx_i_arr, idx_a_arr)
    else:
        ti = _load_text(emb_root, "flickr8k", splits["flickr8k"], witness, idx_i_arr)
        ta = _load_text(emb_root, "clotho",   splits["clotho"],   witness, idx_a_arr)
        sim = _l2norm(ti) @ _l2norm(ta).T
    _WITNESS_SIM_CACHE[key] = sim
    return sim


def _semantic(emb_root, splits, idx_i, idx_a, pi, bridge, ks,
              B, ci, rng, prefix: str = "", T=None,
              lite: bool = False) -> dict:
    ...
    (Jaccard over caption tokens). With `lite=True`, only the H_FGW
    essentials are computed (`recall@1_<w>` + CI, `mrr_<w>`); everything
    else is skipped to keep the eval slim and fast.
    """
...
def _evaluate_combo(combo, plan_dir, emb_root, splits,
    ...
    out = {**combo, **transport}
    out.update(_structural(C_i, C_a, pi, B, ci, rng, lite=lite))
    out.update(_triplet_agreement(C_i, C_a, pi, n_triplets, B, ci, rng, lite=lite))
    out.update(_semantic(emb_root, splits, idx_i, idx_a, pi, bridge, ks,
                         B, ci, rng, T=T, lite=lite))
    # Structural null: depends only on (C_i, C_a), so compute once per combo
    # rather than once per π.
    out.update(_structural_null(C_i, C_a, null_K, ci, rng, lite=lite))

    if with_baseline:
        pi_b = _text_only_pi(emb_root, splits, idx_i, idx_a, bridge)
        out.update(_structural(C_i, C_a, pi_b, B, ci, rng,
                               prefix="baseline_", lite=lite))
        ...
        out.update(_semantic(emb_root, splits, idx_i, idx_a, pi_b, bridge, ks,
                             B, ci, rng, prefix="baseline_", T=None, lite=lite))
...
def _evaluate_one(results_path: Path, args) -> Path:
    ...
    # Reset the witness-sim cache between files: idx_i, idx_a typically vary
    # by seed across bundles, so cache hits only matter within one file.
    _WITNESS_SIM_CACHE.clear()
...
def main():
    ...
    ap.add_argument("--lite",         action="store_true",
                    help="H_FGW headline mode: keep only recall@1 (+CI) and mrr "
                         "per witness, pearson_dist_corr (+CI) + null_pearson_*, "
                         "triplet_agreement, and transport stats. Skips spearman, "
                         "caption_sim_*, mean/median_rank, soft_recall, recall@5/10. "
                         "Defaults to B=200, null_K=50, ks=[1] in this mode.")
    args = ap.parse_args()

    # Lite-mode default overrides (only when the user didn't pass them explicitly).
    if args.lite:
        if "--bootstrap_B" not in __import__("sys").argv: args.bootstrap_B = 200
        if "--null_K"      not in __import__("sys").argv: args.null_K      = 50
        if "--ks"          not in __import__("sys").argv: args.ks          = [1]
